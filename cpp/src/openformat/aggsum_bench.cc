...
arrow::Status pure_scan(parquet::Encoding::type encoding,
  ...
  parquet::ScanFileContentsArrowSum(columns, BATCH_SIZE, pq_file_reader.get(), &compute_time, bitmap);
  ...
  //   auto item = total_array[i];
  //   std::shared_ptr<Buffer> array_tmp = std::make_shared<Buffer>(reinterpret_cast<uint8_t*>((*bitmap).data()+i*ROW_GROUP_SIZE/8),ROW_GROUP_SIZE/8);
  //   item->SetBuffer(0, array_tmp);
  //   total_array_vector.push_back(std::move(item->ToArray()));
  // }

  // for(int i =0;i<row_group_num;i++){
  //     auto result = arrow::compute::Sum(total_array_vector[i], arrow::compute::ScalarAggregateOptions::Defaults(), ctx);
  //     // const int64_t* result_sq = result->array()->GetValues<int64_t>(1);
  //       // std::cout<<result_sq[0]<<std::endl;
  // }
  // compute_time += ((fsec)(Time::now() - begin)).count();
...
arrow::Status RunMain(int argc, char** argv) {
  ...
  std::vector<uint8_t> bitmap = load_bitmap("/root/arrow-private/cpp/Learn-to-Compress/data/bitmap_lyh/"+ bitmap_name);
  ...
  if (gen_data_flag) {
    ...
    if (encoding == "PLAIN") {
      ...
      ARROW_RETURN_NOT_OK(pure_scan(parquet::Encoding::DELTA, &bitmap));
    } else if (encoding == "DICT") {
