...
arrow::Status filter_groupby(parquet::Encoding::type encoding, int64_t filter_val,
  ...
  if(bitmap_groupby){
      ...
      std::vector<std::shared_ptr<Array>> groupbykey;
      std::vector<std::shared_ptr<Array>> sumkey;
      for(int i = 0; i< total_array.size(); i++){
        std::shared_ptr<Buffer> array_tmp = std::make_shared<Buffer>(reinterpret_cast<uint8_t*>(bitmap.data()+(int)(i/2)*ROW_GROUP_SIZE/8),ROW_GROUP_SIZE/8);
          std::vector<std::shared_ptr<Buffer>> buffers_total = {array_tmp, total_array[i]->GetBuffer(1)};
          std::shared_ptr<ArrayData> data = ArrayData::Make(::arrow::int64(), ROW_GROUP_SIZE,
                                                std::move(buffers_total), remain_bits[(i/2)], 0,total_array[i]->compression_type);
        if(i%2==0){
          groupbykey.push_back(std::move(MakeArray(data)));
        }
        else{
          sumkey.push_back(std::move(MakeArray(data)));
          // sumkey.push_back(std::move(total_array[i]->ToArray()));
        }
      }

      ChunkedArray chunkedgroupbykey(groupbykey);
      ChunkedArray chunkedsumkey(sumkey);
      ...
      auto result = GroupBy({chunkedsumkey.Slice(0)}, {chunkedgroupbykey.Slice(0)}, aggregates);
