...
arrow::Status filter_groupby(parquet::Encoding::type encoding, int64_t filter_val,
  ...
  if(bitmap_groupby){
      ...
      std::vector<std::shared_ptr<Buffer>> buffers_total = {nullptr, array_tmp};
      std::shared_ptr<ArrayData> data = ArrayData::Make(::arrow::boolean(), DATA_SIZE,
                                                    std::move(buffers_total), 0, 0,CODEC::PLAIN);
      auto indice =  MakeArray(data);
      ...
      //todo: groupby
      auto col1 = arrow::compute::Filter(groupby_col, indice, arrow::compute::FilterOptions::Defaults(), ctx)->array();
      auto col2 = arrow::compute::Filter(sum_col, indice, arrow::compute::FilterOptions::Defaults(), ctx)->array();
      std::vector<Aggregate> aggregates;
      aggregates.push_back({"hash_sum", std::move(nullptr),
                          "agg_" + std::to_string(0),"hash_sum"});

      auto result = GroupBy({col2}, {col1}, aggregates);
