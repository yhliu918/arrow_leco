...
//   std::vector<uint8_t> value_return(sizeof(uint64_t)*DATA_SIZE);
  ...
  std::vector<uint8_t> values(bitpos->size() * sizeof(uint64_t));
  auto begin = stats::Time::now();
  ARROW_ASSIGN_OR_RAISE(auto input, arrow::io::ReadableFile::Open(
                                        parquet_name, arrow::default_memory_pool()));
  ...
  if (bitpos == nullptr) {
    ...
    //                                       *bitpos, values.data(), &compute_time);
    // } else {
    parquet::ScanFileContentsBitposOld(columns, BATCH_SIZE, pq_file_reader.get(), *bitpos,
                                    rep_levels, def_levels, values, &compute_time);
  ...
  int64_t* res = reinterpret_cast<int64_t*>(values.data());
